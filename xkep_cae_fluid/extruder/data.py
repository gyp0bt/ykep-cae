...
class ParticleTrackInput:
    ...
    cfl : float
        ...
        既定 1.0 は G4b の構成（z_axial = 0.2 m、周回 1〜2 回）で ⟨t⟩ が 1.5% に
        入る値。**長い区間を追うなら 0.25 以下にする。** ψ の双一次補間場は
        セル境界で速度勾配が不連続なので、セル 1 個分を跨ぐ RK4 ステップは
        流線を横切る誤差を残し、周回を重ねると粒子が壁際の遅い領域へ流れ込む。
        閉チャネル H = 1 mm、z_axial = 0.5 m（周回 ≈ 5）の実測（G5 プローブ）:
        |Δψ|/ψ_max の中央値が cfl 1.0 で 8%（粗格子）・2.8%（32×80）、
        0.25 で 1.7%・0.5%。⟨t⟩/t̄_theory は 1.15 → 1.00 に戻る。
        0.1 まで下げると全粒子がステップ上限に掛からず、詰まる粒子が消える分
        壁時計はむしろ短い（32×80 で 115 s → 22 s）。
    max_steps : int

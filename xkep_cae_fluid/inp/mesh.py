...
（C3D4 系）、2D なら四辺形（CPS4 系）/ 三角形（CPS3 系、厚さ ``depth_2d`` で押し出して
...
from xkep_cae_fluid.core.data import CELL_TYPE_HEX, CELL_TYPE_TET, CELL_TYPE_WEDGE, MeshData
...
# Abaqus の面番号 S1.. → 局所節点 index（四面体 C3D4、楔 C3D6。六面体は grid._HEX_FACES）
...
_FACES_3D: dict[int, dict[str, tuple[int, ...]]] = {4: _TET_FACES, 6: _WEDGE_FACES, 8: _HEX_FACES}
_CELL_TYPE_3D: dict[int, int] = {4: CELL_TYPE_TET, 6: CELL_TYPE_WEDGE, 8: CELL_TYPE_HEX}
...
def _element_connectivity(
    ...
    allowed = (4, 6, 8) if is3d else (3, 4)
    for b in case.elements:
        if b.nodes_per_element not in allowed:
            raise UnsupportedMeshError(
                f"要素 {b.element_type}（{b.nodes_per_element} 節点）は未対応"
                f"（3D は四面体 4 / 楔 6 / 六面体 8 節点、2D は三角形 3 / 四辺形 4 節点）"
            )
    ...
    for b in case.elements:
        for row in b.connectivity.tolist():
            try:
                cells.append([node_index[int(n)] for n in row])
            except KeyError as exc:
                raise UnsupportedMeshError(
...
class InpMeshProcess(PreProcess["InpMeshInput", "InpMeshResult"]):
    """``*NODE`` / ``*ELEMENT``（六面体 / 楔 / 四面体、2D 四辺形 / 三角形）を面ベースの非構造 ``MeshData`` にする PreProcess."""

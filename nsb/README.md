...
## 壁セル: 厚み h ≤ `h_solid` のセルを解かない（status-48）

閉塞域は従来 Brinkman 抗力 12μ/h² を大きくして「解いて止めて」いた。`NSBInput.h_solid` を
与えると、代わりに**その面を no-slip 壁にして方程式ごと落とす**。

```python
inp = NSBInput(nx=nx, ny=ny, lx=lx, ly=ly, h=h, bc=bc, h_solid=1.0e-4)  # h <= 1e-4 m を壁に
```

幾何としては「境界を領域の 4 辺だけでなく内部の面にも置く」ことで、面ごとの種別
`wall_x` / `wall_y`（0: 内部面 / 1: 右・上セルが流体 / 2: 左・下セルが流体 / 3: 両側固体）で表す。
壁面 1 枚の役割は 4 辺の WALL 面とまったく同じ:
...
固体セルの行は残差 0・ヤコビアン単位行に置き換わるので δ = 0 に固定され、実質的に解かれない。
圧力基準に繋がらない流体の孤立塊は行列が特異になるので固体に落とす（流入だけある塊は例外）。

効くのは系が小さくなること（trama 400×233 で自由度 279600 → 74664）**だけではない**。
流路 2493 : 閉塞 3.6e8 という 1.4×10⁵ 倍の抗力コントラストが消えるので、ILU も AMG も効くようになる。

uturn 144×96 U=1 での実測（閉塞 h=1e-5 の Brinkman 版 → 壁セル版）:

| | 反復 | 時間 | GMRES / KSP |
|---|---|---|---|
| nsb `jfnk`（PARDISO LU） | 60 未収束 → **28 収束** | 39.5 → **7.9 s** | 1230 → 328 |
| nsb `jfnk_simple`（ILU + AMG） | 23 → 29 | 28.1 → **11.0 s** | 2067 → 1302 |
| nsbp（ASM + ILU(2)） | 27 → 27 | 12.2 → **4.2 s** | 811 → 395 |

物理は一致する: 圧力差 17397 → 17576 Pa（1.0% 差。壁は片側 2 倍勾配なので抗力がわずかに大きい）、
最大流速 1.9081 → 1.9074 m/s（0.04% 差）。

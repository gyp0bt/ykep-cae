...
probe で値表示。`FORMAT=` を書かなければ messi のある環境では HTML が自動で出る。詳細は [設計文書](docs/design/mirador-export.md)。

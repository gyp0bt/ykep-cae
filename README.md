...
**59 テスト** -- 2026-03-31 疎行列ソルバー + フィンアレイ2D/3D + CI整備 | 契約違反 **0件** | [ロードマップ](docs/roadmap.md) | [ステータス一覧](docs/status/status-index.md)
...
|   +-- solver.py      # HeatTransferFDMProcess (ヤコビ/ガウスザイデル/疎行列)
|   +-- solver_vectorized.py  # NumPy ベクトル化ヤコビ法
|   +-- solver_sparse.py      # SciPy 疎行列ソルバー (直接解法/BiCGSTAB)

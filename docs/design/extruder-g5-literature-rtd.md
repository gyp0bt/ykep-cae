...
数値評価は ξ ∈ (2/3, 1) の開区間で中点則。ξ→1 では ξ_c→0 で ŵ_c/|û_c| → (1+3r)/2
（0/0 を極限で処理）、t → ∞（根元に張り付く）なので F は 1 に漸近する裾を持つ。
ξ_c は g(ξ_c) = g(ξ) を (0, 2/3) で二分法（ベクトル化）で解く。

再導出の数値（n_ξ = 4000, 本設計時の確認）: t_min/t̄ ≈ 0.757, p10/t̄ ≈ 0.759,
p50/t̄ ≈ 0.830, p90/t̄ ≈ 1.335。文献に引用される「t_min/t̄ ≈ 0.75」と整合。
...
| 普遍性 | r ∈ {0, −0.3, −0.7} で p10/p50/p90 比が一致 | \|差\| / 1e-6 |
| t_min/t̄ | 0.757（n_ξ 収束値、n_ξ=4000 と 16000 で一致） | \|差\| / 2e-3 |

...
#### 6.1a [status-21 予定]: ScalarTransportProcess 新設

- 新規: `xkep_cae_fluid/scalar_transport/`
  - `data.py`: `ScalarFieldSpec`, `ScalarBoundarySpec`（Dirichlet/Neumann/Robin）, `ScalarTransportInput`, `ScalarTransportResult`
  - `solver.py`: `ScalarTransportProcess` — 既存 `core/strategies/` の対流/拡散/TVD/非直交補正を **再利用**
- 既存エネルギー方程式のアセンブリ構造を共通ヘルパーへ切り出し（コピーではなく共通化）
- プログラムテスト: 純拡散解析解（1D 誤差関数解）、1D Taylor-Green 対流保存則

#### 6.1b [status-22 予定]: NaturalConvection 統合
...
#### 6.2a [status-23 予定]: AquariumGeometryProcess
...
#### 6.2b [status-24 予定]: HeaterProcess + 最小デモ

- 新規: `xkep_cae_fluid/aquarium/heater.py`
  - 定温制御（ヒステリシス ON/OFF）と定熱流束の 2 モード
  - 既存 `q_vol` 経路に流し込む
- 新規: `examples/aquarium_heater_natural_convection.py`
  - ヒーター単独 → 水槽内温度分布と自然対流循環の可視化（PNG 出力）
- **ユーザーが最初に価値を感じるデモ**

### Phase 6.3: 外部フィルター（強制対流統合、2 PR）

#### 6.3a [status-25 予定]: InternalFaceBC

- 既存 `INLET_VELOCITY` / `OUTLET_PRESSURE` は領域外面のみ。水槽内部任意セル面に適用できるよう `InternalFaceBC` を `natural_convection/assembly.py` に追加
- Eheim 吐出方向（ベクトル指定）と吐出量 Q [L/h]

#### 6.3b [status-26 予定]: 強制＋自然対流統合例

- 新規: `examples/aquarium_filter_circulation.py`
  - インレット（上部）＋アウトレット（底床直上）＋ヒーター
- 検証: 質量保存（流入=流出、< 1%誤差）、平均流速 Q/A プロファイル

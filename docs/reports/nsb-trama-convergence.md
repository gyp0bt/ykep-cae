...
### 14.6b 剥離剪断層が崩れ始める位置 — 定量（2026-09-11 追記）

§14.6 の「遷移開始位置のずれ」を数で押さえた。上側水平区間（90 度の曲がりは x = 130 mm）で、
断面の最大変動 RMS がしきい値を超える x を両者で比べる。

<p align="center"><img src="../../experiments/nsb/results/trama_figs/f21_lead_onset.png" width="100%"></p>

| しきい値 | nsb | OpenFOAM | **ずれ** |
|---|---|---|---|
| 5% u_mean (0.038 m/s) | 218.2 mm | 141.8 mm | **+76.5 mm** |
| 10% u_mean (0.076 m/s) | 242.2 mm | 158.2 mm | **+84.0 mm** |
| 15% u_mean (0.114 m/s) | 252.8 mm | 176.2 mm | **+76.5 mm** |
| 20% u_mean (0.153 m/s) | 到達せず | 180.8 mm | — |

**しきい値をどこに置いても +77〜84 mm（流路幅 2.2〜2.4 個ぶん）で一定**、向きは常に nsb が下流。
曲がりからの発達距離で見ると OF 12〜46 mm に対し nsb 88〜123 mm ＝ **2.5〜3 倍**。
nsb はこの区間で 20% u_mean に到達すらしない。

gyp さんが別環境の Fluent 比較で見ている「剥離位置が毎回ちょっとずれる」と**符号も桁も一致する**。
偶然の食い違いではなく**一貫した片側バイアス**で、nsb 側が遅れる。
上流ほど余計な減衰が少ないので、これは **nsb 側の実効粘性が高い**ことを示す。

流速・圧力・変動 RMS の場は次の 2 枚（拡大は曲がり直後）。圧力は両者ほぼ見分けがつかない
（差は最大 ±700 Pa ＝ span の 8%）。

<p align="center"><img src="../../experiments/nsb/results/trama_figs/f20_lead_fields.png" width="100%"></p>
<p align="center"><img src="../../experiments/nsb/results/trama_figs/f22_lead_zoom.png" width="100%"></p>

作図と測定は `experiments/nsb/plot_lead_fields.py`。
...
## 15. 剥離開始位置のずれはどこから来るのか（2026-09-11、gyp さん「剥離開始位置調査」）

§14.6b は「nsb の剥離剪断層が崩れ始める位置が OpenFOAM より +77〜84 mm 下流」という**位置**を
測った。ここではその位置を作っている量まで降りて、どの部品がずれを生んでいるかを分ける。

### 15.1 舞台: 崩れ始める位置は 3 つの量で決まる

曲がりを出た剪断層は**対流不安定**で、擾乱は流れに乗って流されながら空間的に増幅する。
断面の変動 RMS を A(x) とすると、飽和するまでは

    A(x) = A_0 · exp[ σ · (x − x_bend) ]

で、しきい値 A_th を横切る位置は

    x_onset(A_th) = x_bend + ln(A_th / A_0) / σ

になる。つまり容疑者は 3 人しかいない。

<div style="overflow-x:auto"><svg viewBox="0 0 960 300" width="100%" style="min-width:640px" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="13">
  <defs><marker id="a15" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#52514e"/></marker></defs>
  <line x1="70" y1="250" x2="900" y2="250" stroke="#52514e" stroke-width="1.5" marker-end="url(#a15)"/>
  <line x1="70" y1="250" x2="70" y2="30" stroke="#52514e" stroke-width="1.5" marker-end="url(#a15)"/>
  <text x="880" y="272" text-anchor="end">曲がりからの距離 x − x_bend</text>
  <text x="78" y="26">変動 RMS（対数）</text>
  <line x1="70" y1="90" x2="900" y2="90" stroke="#bbb" stroke-dasharray="4 3"/>
  <text x="906" y="94" font-size="11" fill="#777">A_th = 20% u_mean</text>
  <line x1="70" y1="140" x2="900" y2="140" stroke="#bbb" stroke-dasharray="4 3"/>
  <text x="906" y="144" font-size="11" fill="#777">10%</text>
  <line x1="70" y1="180" x2="900" y2="180" stroke="#bbb" stroke-dasharray="4 3"/>
  <text x="906" y="184" font-size="11" fill="#777">5%</text>
  <path d="M70,205 L330,90 L900,80" fill="none" stroke="#1a1a1a" stroke-width="2.5"/>
  <text x="250" y="80" fill="#1a1a1a" font-weight="bold">OpenFOAM: 急に立つ（σ 大）</text>
  <path d="M70,232 L840,90 L900,88" fill="none" stroke="#2a78d6" stroke-width="2.5"/>
  <text x="540" y="185" fill="#2a78d6" font-weight="bold">nsb: 寝ている（σ 小）</text>
  <circle cx="70" cy="205" r="4" fill="#1a1a1a"/><circle cx="70" cy="232" r="4" fill="#2a78d6"/>
  <text x="18" y="209" font-size="11" fill="#1a1a1a">A_0</text>
  <text x="18" y="236" font-size="11" fill="#2a78d6">A_0</text>
  <line x1="256" y1="140" x2="256" y2="250" stroke="#1a1a1a" stroke-dasharray="3 3"/>
  <line x1="600" y1="140" x2="600" y2="250" stroke="#2a78d6" stroke-dasharray="3 3"/>
  <line x1="256" y1="268" x2="598" y2="268" stroke="#eb6834" stroke-width="2" marker-end="url(#a15)"/>
  <text x="428" y="288" text-anchor="middle" fill="#eb6834" font-weight="bold">これが測っていた +84 mm</text>
  <rect x="620" y="200" width="290" height="46" rx="6" fill="#fff6e8" stroke="#eb6834"/>
  <text x="632" y="219" font-size="12">容疑者 1: 種 A_0（曲がりを出た時点の擾乱）</text>
  <text x="632" y="238" font-size="12">容疑者 2: 増幅率 σ　容疑者 3: 元の剪断層の厚み θ</text>
</svg></div>

θ が容疑者なのは、混合層の増幅率が σ·θ ≈ 一定（厚い層ほど寝る）という関係で決まるから。
以下、この 3 つを順に潰す。

### 15.2 入ってくる場は同じ — 剪断層の厚みは両者で一致する

助走脚（`--variant lead` で継ぎ足した入口の直線 172.5 mm）は変動 RMS がほぼ 0 なので、
時間平均場の比較が**決定論的な場の比較**になる。刳り抜きポート（直径 28.5 mm）の両脇 3 mm から
出る 2 本の噴流の、内側剪断層の運動量厚み θ = ∫f(1−f)ds を区間に沿って測る。

| y [mm]（助走脚に沿って） | 145 | 169 | 191 | 214 | 238 | 261 |
|---|---|---|---|---|---|---|
| θ nsb [mm] | 1.125 | 1.046 | 0.994 | 0.979 | 0.964 | 0.954 |
| θ OpenFOAM [mm] | 1.235 | 1.081 | 1.013 | 1.004 | 0.988 | 0.979 |
| 比 nsb/OF | 0.91 | 0.97 | 0.98 | 0.98 | 0.98 | 0.97 |

**nsb の方がむしろ 2〜9% 薄い。** そして注目すべきは絶対値で、θ ≈ 0.95〜1.13 mm は
**格子 Δx = 1.5 mm の 0.65 倍**、しかも 120 mm 走ってもほとんど広がらない。
物理の拡散だけなら √(ν t) = √(3e-6 × 0.043) = 0.36 mm で、Δx の 1/4 しかない。

つまり **剪断層の厚みは物理ではなく格子が決めており、両コードで同じ**。容疑者 3 は消える。
同時にこれは重い意味を持つ ― **この Δx では増幅率もスキームが決めている**。

### 15.3 分解: 効いているのは種ではなく増幅率

上側水平区間で断面最大の変動 RMS を x に沿って取り、指数域を回帰する（`trama_shear_diag.py`）。

| | σ [1/mm] | e 倍長 | A_0 / u_mean | 5% 到達 | 10% 到達 | 15% 到達 |
|---|---|---|---|---|---|---|
| nsb | 0.0212 | 47.2 mm | 0.0048 | 105.6 mm | **129.6 mm** | 140.1 mm |
| OpenFOAM | 0.0512 | 19.5 mm | 0.0088 | 29.1 mm | **45.6 mm** | 63.6 mm |
| 比 | **2.42** | | 1.86 | | **+84.0 mm** | |

10% しきい値のずれを 2 項に割ると **種の差 +29.2 mm ＋ 増幅率の差 +67.0 mm = +96.2 mm**
（実測 +84.0 mm。回帰の外挿ぶん過大に出る）。**7 割は増幅率**で、種は 3 割。
しきい値をどこに置いてもずれが +77〜84 mm でほぼ一定だったのは、
種の差（平行移動）と増幅率の差（傾き）がこの範囲でたまたま似た寄与になっていたため。

<p align="center"><img src="../../experiments/nsb/results/trama_figs/f23_lead_shear.png" width="100%"></p>

### 15.4 nsb が擾乱を殺している場所 1: リミターが剪断層に沿って潰れる

nsb の 2 次風上は、上流セルの値に外挿量 e = ½Δ·ψ·∂φ/∂n を足す。ψ は Venkatakrishnan
リミターで、**セルに 1 つ**しかなく、x 方向と y 方向の外挿に同じ値が掛かる（`nsb/fastres.py`）。
ψ は「外挿した面値が隣接 4 セルの最大・最小をはみ出さない」ことを要求して決まる。

ここに負のフィードバックがある。**成長中の波の峰は、まさに「隣より大きい値」＝ 新しい極値**なので、
波が育つほどリミターが強く踏む。しかも ψ がセルに 1 つなので、横断方向の極値が
**流れ方向の外挿まで一緒に潰す**。潰れたセルは 1 次風上に退化し、その数値粘性は
ν_num = ½|u|Δ(1−ψ) になる。

実測（曲がり〜+200 mm の剪断層領域、保存場から ψ を再構成）:

| | ψ 中央値 | ψ 下位 10% | ψ<0.5 の割合 | ν_num/ν_phys 中央値 | 同 上位 10% |
|---|---|---|---|---|---|
| nsb 既定（K=5） | 0.724 | 0.097 | 32.2% | **31.5** | **350.5** |

**剪断層のセルの 3 分の 1 が実質 1 次風上で走っており、そこでの実効粘性は物理の 30〜350 倍。**
下の図の上段で、ψ が流れ方向の縞になって落ちているのがその剪断層。

<p align="center"><img src="../../experiments/nsb/results/trama_figs/f24_lead_limiter_rc.png" width="100%"></p>

### 15.5 nsb が擾乱を殺している場所 2: 非定常の Rhie–Chow が時間微分の対角を落としている

Rhie–Chow の面速度補正は d_f = V/a_P に比例する。a_P は運動量方程式の対角なので、
後退 Euler なら本来 ρV/Δt を含む（OpenFOAM の 1/A がそうなっている）。
`nsb/solver.py` の定常ソルバーは擬似時間対角を入れているのに、
**`nsb/unsteady.py` だけがそれを渡していなかった**（`compute_state(x, scheme, venkat_k)`）。

効き方は場所で全く違う。a_P ≈ Σ|対流流束| + 拡散 + 抗力 なので、

- 主流（|u| ≈ 0.76 m/s、CFL ≈ 0.5）: a_P ≈ ρV/Δt → d_f は 2 倍過大
- **剥離域（|u| ≈ 0.05 m/s）: a_P ≪ ρV/Δt → d_f は 10 倍以上過大**

上の図の下段が (a_P + ρV/Δt)/a_P の場で、**曲がり内側の剥離域だけが飽和（≥10）している**。
実測の中央値 3.0・上位 10% 13.3。Δt を小さくしても消えない人工散逸が、
不安定性が育つ場所にだけ乗っていたことになる。

`NSBSettings.rc_with_pseudo_time` を非定常でも効かせるようにした（既定は従来どおり False、
`trama_case.py --rc-dt`）。検査は `tests/test_nsb_unsteady_rc.py` 3 件。

### 15.6 OpenFOAM 側の非定常はソルバーのノイズではない

「OF の方が 1 ステップあたりの収束が緩いから擾乱が乗っているだけ」という疑いは潰しておく。
`nOuterCorrectors` を 3 → 20 にして同じ計算をすると、変動 RMS の包絡はほとんど動かない。

| 曲がりからの距離 [mm] | 5.7 | 17.7 | 29.7 | 41.7 | 53.7 |
|---|---|---|---|---|---|
| RMS/u_mean（nOuter=3） | 0.0017 | 0.0473 | 0.3073 | 0.5041 | 0.5480 |
| RMS/u_mean（nOuter=20） | 0.0017 | 0.0499 | 0.3188 | 0.5225 | 0.5283 |

**OF の非定常は自励的で、離散系が本当に持っている不安定性。**

### 15.7 掃引の台: 入口だけを切り出した縮小ジオメトリ

本番（400×233、流体 28485 セル、1.68 s/step、3000 歩で 1.4 時間）で掃引はできないので、
**入口部分だけを切り出した縮小ケース**を作った（`experiments/nsb/patterns/lead_stub.json`）。
助走脚 172.5 mm + 90 度の曲がり + 直線 340 mm。流路幅・Δx・Δt・流量・ポートは本番と同一で、
`--lx / --ly / --scale` を nsb 側と OpenFOAM 側の両ランナーに足して格子を揃えた。

| | 格子 | 流体セル | 1 歩 | 1 走行 |
|---|---|---|---|---|
| 本番 | 400×233 | 28485 | 1.68 s | 1.4 時間 |
| 縮小 | 274×162 | 7857 | **0.52 s** | **10 分** |

**再現するもの**: nsb の増幅率。σ = 0.0207 /mm（本番 0.0212）で、**切り詰めても変わらない**。
剪断層の厚み θ も一致する。

**再現しないもの**: OpenFOAM 側の絶対値。縮小ケースでは出口ポートの変動が上流に効いて
曲がり直後から強く励振され、5% → 20% が 4.5 mm で立ってしまう（本番は 34 mm）。
なので **nsb の設定同士を同じ台で比べる用途に限って使い、本番との突き合わせは最後だけ**にする。
（この非対称そのものが示唆的で、**nsb の σ は下流の構成に鈍く、OF の σ は敏感**。
nsb 側は物理の不安定性ではなくスキームの減衰で律速されている、と読める。）

### 15.8 掃引: 数値減衰を減らすと擾乱はどう育つか

同じ初期場（1000 歩の助走）から、スキームだけを変えて 1200 歩（1.2 s）走らせ、
最後の 0.7 s を時間平均する。指数回帰は飽和が早いケースで当てにならないので、
**5% → 20% u_mean に育つのに要る距離**と**飽和 RMS** を主指標にする。

| 変種 | ε² = (KΔ)³ | ψ 中央値 | ψ 下位 10% | ψ<0.5 の割合 | ν_num/ν_phys | 5%→20% の距離 | 飽和 RMS/u_mean |
|---|---|---|---|---|---|---|---|
| E `--convection fou` | — | — | — | — | 14.2 | **5% に届かない** | 0.045 |
| A 既定（K=5） | 4.2e-07 | 0.830 | 0.136 | 26.0% | 31.0 | 48.0 mm | 0.511 |
| B `--venkat-k 50` | 4.2e-04 | 0.889 | 0.259 | 18.6% | 22.5 | 51.0 mm | 0.531 |
| D `--rc-dt`（K=5） | 4.2e-07 | 0.837 | 0.133 | 24.9% | 28.9 | 43.5 mm | 0.645 |
| C `--venkat-k 500` | 4.2e-01 | 0.994 | 0.929 | 4.1% | 1.0 | 25.5 mm | 0.681 |
| **OpenFOAM** | （BJ 相当） | — | — | — | — | **4.5 mm** | **0.733** |
| H `--venkat-k 1500` | 3.4e+01 | 1.000 | 0.996 | 0.0% | 0.0 | 3.0 mm | 1.305 |
| F `--venkat-k 5000` | 4.2e+02 | 1.000 | 1.000 | 0.0% | 0.0 | 1.5 mm | 1.525 |

<p align="center"><img src="../../experiments/nsb/results/trama_figs/f23_stub_shear.png" width="100%"></p>

右のパネルがそのまま結論の図で、下から順に FOU（桃、平ら）／既定 K=5・K=50・`--rc-dt`（青・橙・茶）／
K=500（緑）／OpenFOAM（黒）／K=1500・K=5000（赤・紫、OF を追い越す）と並ぶ。
左と中央のパネルは、**どの設定でも助走脚の平均場と剪断層の厚みはほとんど変わらない**ことを示している
（リミターは剥離域でしか効いていない）。

**OpenFOAM は K=500 と K=1500 の間に挟まる。** nsb の既定は 2〜3 桁きつい側に外れており、
リミターを完全に外すと今度は緩すぎる（飽和 RMS が OF の 1.8 倍 ＝ 単調性を失った振動）。
`cellLimited Gauss linear 1`（Barth–Jespersen）はちょうどその中間にいる。

読み方はこの 3 つ。

- **効いているのは ψ の中央値ではなく「潰れた尾」。** K=50 は中央値を 0.830 → 0.889 に上げ、
  ν_num を 31 → 22.5（27% 減）にするが、育つ距離は 48 → 51 mm と**まったく動かない**。
  一方 K=500 は ψ<0.5 のセルを 26.0% → 4.1% に減らし、そこで初めて 25.5 mm と半減する。
  **剪断層に乗っているごく一部の「ψ→0 のセル」が増幅率を握っている**。
  だから K は桁で動かさないと反応しない。
- `--rc-dt` も同じ向きに効く（48 → 43.5 mm、飽和 0.51 → 0.645）。効き幅は小さいが、
  リミターの K と違って**物理的に正しい形に直す**変更なので、独立に入れる価値がある。
- **FOU は論外。** 擾乱が消える（飽和 RMS が OF の 1/16）。§14.6 で「FOU にすれば残差の頭打ちが
  下がる」と書いたが、それは**不安定性を潰して定常に寄せているだけ**だと分かる。ここで訂正する。

### 15.9 なぜ「K を 100 倍にしないと効かない」のか — ε² が無次元でない

掃引で「K=50 は無反応、K=500 で急に効く」という不連続な効き方をしたのは偶然ではない。
Venkatakrishnan のリミターは

    ψ = (Δ_max² + ε² + 2 Δ_f Δ_max) / (Δ_max² + 2 Δ_f² + Δ_f Δ_max + ε²)

で、ε² は「この程度の変動は平らとみなす」という**しきい値**である。nsb はこれを
`eps2 = (venkat_k · min(dx, dy))³`（`nsb/fastres.py:253`）で作っている。これは
**Venkatakrishnan の原論文が無次元変数（速度は自由流で、長さは代表長で割ってある）を前提**に
していることを引き継いだ形で、**次元のある量にそのまま使うと意味が変わる**。

| K | 現行 (K·Δ)³ | 無次元化した形 K³(Δ/w)² u_mean² |
|---|---|---|
| 5（既定） | 4.2e-07 | 6.0e-03 |
| 50 | 4.2e-04 | 6.0e+00 |
| 500 | 4.2e-01 | 6.0e+03 |

剪断層の隣接セル間の速度差は Δu ≈ 0.05〜1 m/s、つまり Δu² ≈ 2.5e-3〜1。
**既定の ε² = 4.2e-7 はこれより 4〜6 桁小さい** ので、実質 ε = 0 として動いている。

ε → 0 の Venkatakrishnan は Barth–Jespersen より**厳しい**（Δ_f = Δ_max のとき BJ が ψ=1 なのに
Venkat は 0.75、Δ_f = 2Δ_max で BJ 0.5 に対し Venkat 0.45）。つまり nsb は、
OpenFOAM の `cellLimited Gauss linear 1`（＝ BJ）が**まったく踏まない滑らかな場所でも踏んでいる**。

これが「格子が mm で速度が m/s」という条件で起きているのがポイントで、
**同じ K でも問題のスケールが変われば効き方が変わる**。無次元化した ε² に直すのが筋
（TODO）。掃引の K=500 は無次元換算で K ≈ 21 に相当し、「かなり緩め」ではあるが非常識ではない。

### 15.10 決定打: Δx を 1.5 → 1.0 mm にすると差が消える

§15.2 で θ ≈ 0.65 Δx（物理の拡散長 0.36 mm より厚い）と分かっている以上、
剪断層は両コードとも格子が決めている。だとすれば **OpenFOAM の Δx = 1.5 mm の答えも
「正解」ではない**はずで、実際そうなっている。そして nsb 側も細分すると別の顔になる。

同じ縮小ケースを Δx 1.5 → 1.0 mm で比べる（`--dx 1.0`、OF も同じ格子）:

| | Δx = 1.5 mm | | Δx = 1.0 mm | |
|---|---|---|---|---|
| | **nsb** | **OpenFOAM** | **nsb** | **OpenFOAM** |
...
| ν_num/ν_phys 中央値（nsb） | 31.0 | — | **8.6** | — |
| ψ<0.5 の割合（nsb） | 26.0% | — | **16.2%** | — |

<p align="center"><img src="../../experiments/nsb/results/trama_figs/f23_stub-dx1_shear.png" width="100%"></p>

**Δx = 1.0 mm では、崩れ始める位置が 2〜4 mm（2〜4 セル）以内で一致する。**
Δx = 1.5 mm の縮小ケースで +46 mm、本番ケースで +84 mm あったずれが、
**格子を 1.5 倍細かくしただけで消える**。

一致するのは**崩れ始める位置**であって、**崩れた後の振幅はまだ揃っていない**（上の図の右）。
曲がりから 25 mm までは 2 本の曲線がほぼ重なるが、その下流で nsb は OF の 1/3〜2/3 に留まる
（区間最大で 1.062 vs 1.332）。飽和振幅は両者とも細分でまだ大きくなり続けているので、
ここは §15.12 の格子収束の追い込みの領分。

機構は §15.4 と地続きで、リミター由来の数値粘性は

    ν_num/ν_phys = ½|u| Δx (1−ψ) / ν

なので、Δx を 1.5 倍細かくすると **(a) Δx がそのまま 1.5 倍効き**、さらに
**(b) 場がセル単位で滑らかになるので ψ が上がる**（中央値 0.830 → 0.925、ψ<0.5 が 26% → 16%）。
合わせて 31.0 → 8.6 の 3.6 倍減で、§15.8 の掃引が示した「反応が始まる閾値」を跨ぐ。

同時に、**θ ≈ 0.67 Δx が両コード・両格子で成り立つ**（1.5 mm で 0.99〜1.01、1.0 mm で 0.67〜0.68）ことが
裏付けられた。剪断層は最後まで格子が決めており、**どちらも格子収束していない**
（OF の飽和 RMS は 0.684 → 1.332 と 2 倍、nsb も 0.511 → 1.062 と 2 倍）。

したがって §14.6b の +84 mm の正体はこうなる。

- **物理としての「正しい剥離開始位置」はまだどちらも出していない**（両者とも未解像）。
- ずれが出るのは、**同じ Δx で nsb の方が数値粘性が大きく、擾乱が育つ閾値の向こう側にいる**から。
  その差を作っているのが §15.9 の ε² の無次元性。
- **Δx を 1 段細かくすると nsb がその閾値をこちら側に跨ぎ、両者は一致する。**

### 15.11 まとめ

- **§14.6b の +84 mm は、種 3 割・増幅率 7 割。** 主犯は「擾乱が育つ速さ」で、
  nsb は OpenFOAM の 1/2.4 でしか育たない。
- **入ってくる場は同じ。** 助走脚の剪断層の運動量厚み θ は両者一致（nsb がむしろ 2〜9% 薄い）。
  しかも θ ≈ 0.65 Δx で物理の拡散長 0.36 mm より厚い ＝ **両コードとも剪断層は格子で決まっている**。
- **nsb の過減衰は 2 か所に局在する。** どちらも剥離域だけに効く:
  1. **Venkatakrishnan リミターが剪断層に沿って潰れる**（ψ<0.5 が 26%、ν_num/ν_phys 中央値 31・上位 10% 354）。
     成長中の波の峰は定義上「隣より大きい新しい極値」なので、育つほど強く踏まれる。
     しかも ψ はセルに 1 個なので、横断方向の極値が流れ方向の外挿まで道連れにする。
  2. **非定常の Rhie–Chow が時間微分の対角 ρV/Δt を落としている**（低速の剥離域で d_f が 10 倍過大）。
     定常ソルバーは入れているのに非定常だけ抜けていた。→ 実装して `--rc-dt` で選べるようにした。
- **原因は ε² が無次元でないこと。** 既定 K=5 の ε² は剪断層の Δu² より 4〜6 桁小さく、実質 ε=0。
  ε→0 の Venkatakrishnan は Barth–Jespersen より厳しいので、
  **OF の `cellLimited 1` が踏まない滑らかな場所でも nsb は踏んでいる**。
  掃引では OpenFOAM が **K=500 と K=1500 の間**に挟まった。
- **効いているのは ψ の中央値ではなく尾。** ψ<0.5 のセルを 26% → 4% に減らして初めて反応する。
- **FOU に逃げてはいけない。** 残差の頭打ちは下がるが、不安定性ごと潰している
  （飽和 RMS が OF の 1/16）。§14.6 の読みをここで訂正する。
- **OpenFOAM 側は健全だが、正解でもない。** `nOuterCorrectors` 3 → 20 で包絡はほとんど動かない
  （＝ 自励的でノイズ由来ではない）が、Δx を 1.5 → 1.0 mm にすると飽和 RMS が 2 倍になる
  （＝ 格子収束していない）。nsb も同じく 2 倍。
- **Δx = 1.0 mm では差が消える。** 崩れ始める位置が 2〜4 mm（2〜4 セル）以内で一致する。
  細分は ν_num = ½|u|Δx(1−ψ)/ν を **Δx で 1.5 倍・ψ の改善で 2.4 倍**、合わせて 31.0 → 8.6 に落とし、
  §15.8 の掃引が示した反応閾値を跨ぐ。**+84 mm は格子解像度の産物**で、
  「どちらのスキームが散逸的か」が Δx = 1.5 mm でだけ症状として出ていた。

### 15.12 次にやること

1. **ε² の無次元化**（優先）。`eps2 = (K·Δ)³` を速度スケールと代表長で規格化した形にする。
   `BrinkmanDiscretization.u_scale` は既にあるので、`eps2 = (K·Δ/L_ref)³·u_scale²` の形が素直。
   既定 K の意味が変わるので、既存ケースの回帰を取り直すこと。
2. **`rc_with_pseudo_time` を非定常の既定にするか**の判断。物理的には正しい形だが、
   既存の非定常結果（§12.7、§14）が動くので、切り替えるなら突き合わせをやり直す。
3. **格子収束の追い込み**。Δx = 1.0 mm で両者は一致するが、**どちらもまだ収束していない**
   （1.5 → 1.0 mm で飽和 RMS が 2 倍）。物理としての剥離開始位置を出すには
   Δx = 0.75 / 0.5 mm まで振る必要がある。縮小ジオメトリならその余地がある。
4. 本番ジオメトリで、上で決めたリミター設定を使って §14.6b の +84 mm がどこまで縮むかを測る。

### 付録 §15: 再現コマンド

```bash
# 既存の保存場だけからの診断（ソルバーを走らせない）
python experiments/nsb/trama_shear_diag.py \
    --nsb "本番=experiments/nsb/results/trama_LEAD-carve-m01-B_fields.npz@5" \
    --of <of>/walls-lead2 --tag lead

# 縮小ジオメトリ（274×162、流体 7857 セル）。助走 1000 歩 → 掃引 1200 歩
STUB="experiments/nsb/patterns/lead_stub.json --variant lead --lx 411 --ly 243 --scale 4.928149 \
      --mass 0.1 --dx 1.5 --port carve --h-solid 1e-4 --unsteady 0.001 --newton-per-step 6 \
      --save-every 0 --linear-solver jfnk --ls 4 --no-stop-at-steady"
~/.claude/hooks/memcap -m 5G -- python experiments/nsb/trama_case.py $STUB \
    --n-steps 1000 --tag STUB-spinup
for v in "A-base:" "B-k50:--venkat-k 50" "C-k500:--venkat-k 500" "H-k1500:--venkat-k 1500" \
         "F-k5000:--venkat-k 5000" "D-rcdt:--rc-dt" "E-fou:--convection fou"; do
  ~/.claude/hooks/memcap -m 5G -- python experiments/nsb/trama_case.py $STUB \
      --n-steps 1200 --avg-start 0.5 \
      --init-from experiments/nsb/results/trama_STUB-spinup_fields.npz \
      ${v#*:} --tag STUB-${v%%:*}
done

# OpenFOAM 参照（同じ縮小ジオメトリ。--n-outer 20 は収束の緩さが種になっていないかの確認）
python experiments/nsb/run_trama_of.py experiments/nsb/patterns/lead_stub.json \
    --geo-variant lead --variant walls --out <of>/of-stub \
    --lx 411 --ly 243 --scale 4.928149 --dx 1.5 --mass 0.1 \
    --transient --end-time-s 2.0 --avg-start 1.0 --max-co 5
python experiments/nsb/run_trama_of.py ... --out <of>/of-stub-tight --n-outer 20
python experiments/nsb/run_trama_of.py ... --out <of>/of-stub-dx1 --dx 1.0 \
    --end-time-s 2.2 --avg-start 1.3

# 掃引のまとめ
python experiments/nsb/trama_shear_diag.py \
    --pattern experiments/nsb/patterns/lead_stub.json --lx 411 --ly 243 --scale 4.928149 \
    --tag stub --x-from 3 \
    --nsb "K5=experiments/nsb/results/trama_STUB-A-base_fields.npz@5" \
    --nsb "K500=experiments/nsb/results/trama_STUB-C-k500_fields.npz@500" \
    --of <of>/of-stub
python experiments/nsb/trama_growth_rate.py \
    base=experiments/nsb/results/trama_STUB-A-base.yaml \
    k500=experiments/nsb/results/trama_STUB-C-k500.yaml
```

...
<div style="overflow-x:auto"><svg viewBox="0 0 980 300" width="100%" style="min-width:640px" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="13">
  ...
  <text x="20" y="208">→ J1 の圧力方向がほぼ平ら、Newton 歩の</text>
  <text x="20" y="226">圧力成分 |δp| が ṁ^2.6 で伸びる（60 → 8.4e6）</text>
  <text x="20" y="244">→ その方向で J1 と真の作用素が食い違う</text>
  <text x="20" y="262" font-weight="bold">厳密歩の真の残差比 1.16 → 81</text>
...
|---|---|---|
| 1. 高 Re | κ₂(J1+τ) は 1.6e12 → 2.1e12 と**ほぼ不変**。変わるのは厳密 J1 歩の真の残差比 1.16 → **81**、圧力歩 60 → **8.4e6** | 主因（J1 と真の作用素の食い違い） |
| 2. 階段 | 階段セルの残差分担 ≈ 1%。直交化版も未収束。直線流路は 0〜45° どの角度でも健全 | 無関係。引き金は蛇行の U ターンで Stokes 出発点が遠いこと（\|R_ref\| 190 vs 直線 7.2） |
| 3. 前処理 | SIMPLE: Givens 0.077 / 真 **30**。LU(J1): 真 **3.3e-3**。ただし LU でも Newton 一歩で残差 **5060 倍** | 主因（SIMPLE が観点 1 の弱点を増幅）。線形解を直しても歩の方向が悪い |
| 追加: ポート | κ は内部 2.1e12 / 壁 2.3e12 で同等。厳密歩の真の残差比は内部 81 / 壁 1.6 | 内部ポートが食い違いを 50 倍増やす |
...
0.0015 kg/s（1/100）は最初の一歩で残差が 6.6 倍に跳ね、CFL 0.03 で立て直して 80 反復で rel 3.7e-6
...
1. **圧力–速度結合が 50 倍弱まる。** a_P が対流（ρ|u|Δy）に支配され、RC 係数 d_f ∝ 1/Re。
   運動量の不釣り合い R（∝ ṁ²）を直すのに必要な圧力歩は R/d ∝ ṁ³ で伸びる。実測の厳密 J1 歩の
   圧力成分 |δp| は 60 → 695 → 1.6e4 → 4.6e5 → 8.4e6（ṁ^2.6）。場の |p| は 4.4e3 Pa なので、
   0.15 では**場の 2000 倍の圧力歩**を J1 が要求している。
2. **その歩に対する真の作用素の応答が J1 と食い違う。** 厳密に (J1+τ) d = −R を解いた d を有限差分の
   ...
   sink の運動量項 q_out·v = C(p − p_sink)·v と RC 質量流束の p 依存が、圧力歩の巨大さに乗じられる。
3. **蛇行だから起きる。** 直線流路は同じ Re・同じ内部ポートでも Stokes 参照場がほぼ NS 解で
   （|R_ref| 7.2、|x| 1.1e5）、Newton 歩が小さく §5 の機構が発火しない。蛇行は U ターン 6 か所で慣性が
   効き、Stokes 出発点が遠い（|R_ref| 190、|x| 7.2e5、最大速さ 6.85 m/s = 平均の 6 倍）。
   大きな R に 1/d ∝ Re が掛かって圧力歩が場の外へ飛ぶ。
4. 条件数そのものは動かない（κ 1.6e12 → 2.1e12）。**「高 Re → 条件数悪化」ではなく「高 Re → J1 の
   圧力方向が平らになり、その方向で Newton 歩が場の外へ飛び出す」**が正しい像。
...
| 同 0.0015 kg/s | 1.3e-6 | 0.21 | 1.6e-3 〜 336 | 棄却は免れるが残差 6.6 倍の悪い歩 |
...
3. **LU(J1) は同じ谷底を J1 と整合して伸ばす**ので、食い違いは低ランク（Ritz 0.39〜26）に留まり 3.3e-3 で解ける。
   ...
   その Newton 一歩で残差が **5060 倍**に跳ね（圧力歩が場の外へ飛ぶ §3）、以後 40 反復かけても rel 1700 に留まる。
   線形ソルバーを完璧にしても、**J1 が指す方向そのものが間違っている**ので救えない。SIMPLE の失敗は
   「悪い歩を棄却する」役を偶然果たしているが、CFL を 1e-38 まで縮めても線形解が直らないので詰む。
...
むしろ速度が凍ると圧力歩だけが残差を担うので |x| = 8.6e7〜3.6e8 と圧力レベル方向へ更に大きく振れ、
...
| 厳密 J1 歩の真の残差比 0.0015 / 0.15 | 1.16 / 81 | **0.15** / 1.6 |
...
ただし壁でも Newton 一歩で残差が 9 倍に跳ねてから棄却の連鎖に入るので、§3 の「圧力歩が場の外へ飛ぶ」
機構は残る（壁 0.15 の厳密歩 |δp| = 2.1e7）。
...
4. **τ の 2 重カウントを直す**（`resid_fn` に `steady_resid` を渡すか `diag_aug` を足さない）。前処理と作用素の
   整合が 1 段良くなる。
5. **圧力歩のガード。** |δp| が場の |p| の数倍を超えたら歩を縮める（線探索）。§3 の ṁ³ 則で飛ぶ歩を
   棄却ではなく縮小で扱う。

...
| 34 | 2026-09-05 | 613 | 3D レンダリング: messi mirador 連携 `MiradorExportProcess`（断面スラブ + 速度矢印 + 残差マップ、`FORMAT=HTML` 自動出力 / `ykep view`）+ messi v0.10.0（要素場カラーマップ Abaqus レインボー・矢印・`.vtk` リーダ・操作パネル畳み込み） |
